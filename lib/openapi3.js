    ...
    for (var p in source.paths) {
        for (var m in source.paths[p]) {
            if ((m !== 'parameters') && (m !== 'summary') && (m !== 'description') && (!m.startsWith('x-'))) {
                ...
                var sMethodUniqueName = (method.operation.operationId ? common.slugify(method.operation.operationId) : common.slugify(m + '_' + p))
                    .split('?')[0].split('/').join('_');
                if (data.options.tocSummary && method.operation.summary) {
                    sMethodUniqueName = method.operation.summary;
                }
                method.slug = common.slugify(sMethodUniqueName.toLowerCase().split(' ').join('-')); // TODO {, } and : ?
